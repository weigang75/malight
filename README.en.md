...
> **v2.0**: package renamed magicpen → **malight**; new filter factory `pen.fx`
> (Photoshop/Illustrator-style filters, chainable); new extension mechanism
> `malight.ext`. Old scripts with `import magicpen` still run (a compatibility shim
> ships in the repo), and `python -m malight.compat old.py new.py` migrates them.
>
> **v2.1**: the "enum + string" dual style everywhere (fonts / colours / option
> values, so typos can't slip through); new path helper package `malight.pathkit`
> (inspect the structure, drag anchors and control points, insert/delete points,
> save/load point data); every class file ships a runnable example; exports print
> the **full path** so you can copy it straight into a shell.
>
> **v2.2**: runtime localisation `malight.i18n` (English by default, one line to
...
> Elements can now be **tweaked after creation**: `el.set_font_size(36)` or
...
Deeper usage lives in [board/core.en.md](malight/board/core.en.md).
...
their members and their meaning: [definitions.en.md](malight/definitions.en.md);
font lookup and embedding: [fonts.en.md](malight/fonts.en.md).
...
The complete API and every message key: [i18n.en.md](malight/i18n.en.md).
...
| Module | Description |
|---|---|
| [compat](malight/compat.en.md) | Mapping tables and a script migrator from the Chinese API to the English one. |
| [definitions](malight/definitions.en.md) | Colors, fonts, paper sizes and every option enum. |
| [ext](malight/ext.en.md) | The extension mechanism: register a third-party toolkit with @toolkit and mount it on a board. |
| [fonts](malight/fonts.en.md) | Font enum plus font lookup, embedding and subsetting helpers. |
| [gradients](malight/gradients.en.md) | Linear and radial gradient definitions. |
| [i18n](malight/i18n.en.md) | Runtime message localisation. |
| [page](malight/page.en.md) | Page setup for printing and PDF export: paper, margins and scaling. |
| [svg_backend](malight/svg_backend.en.md) | A tiny SVG element and serialisation backend (internal). |
| [tools](malight/tools.en.md) | Image, export, geometry, filter and font helpers. |

### malight/board

| Module | Description |
|---|---|
| [containers](malight/board/containers.en.md) | Container elements: g, symbol, use, pattern, marker and a. |
| [core](malight/board/core.en.md) | BoardCore: canvas setup, element registry, page config, backgrounds, lifecycle hooks and finish(). |
| [debug](malight/board/debug.en.md) | Debugging helpers: grids, frames, distance measurements and key points. |
| [effects](malight/board/effects.en.md) | Clipping and masking with SVG clipPath and mask. |
| [filters](malight/board/filters.en.md) | The filter factory behind pen.fx and pen.filter. |
| [fx](malight/board/fx.en.md) | FilterChain, a chainable builder for Photoshop-style filter stacks. |
| [gradients_mixin](malight/board/gradients_mixin.en.md) | Gradients: SVG linearGradient and radialGradient, plus rainbow and gold shortcuts. |
| [images](malight/board/images.en.md) | Images: bitmap `<image>` placement and SVG import. |
| [layout](malight/board/layout.en.md) | Arranging elements horizontally, vertically, in a grid or around a circle. |
| [paths](malight/board/paths.en.md) | Paths and connectors: path, polyline, tables, arrows and wave lines. |
| [repeat](malight/board/repeat.en.md) | Repetition: grid, circular and linear. |
| [shapes](malight/board/shapes.en.md) | Basic shapes, each method named after its SVG element. |
| [style](malight/board/style.en.md) | The style helper (pen.style) for CSS classes, the global stylesheet and font embedding. |
| [text_board](malight/board/text_board.en.md) | Text: SVG text, textPath and text-to-path conversion. |

### malight/elements

| Module | Description |
|---|---|
| [base](malight/elements/base.en.md) | The Element base class and the helpers shared by every element. |
| [circle](malight/elements/circle.en.md) | CircleElement, created by pen.circle. |
| [clippath](malight/elements/clippath.en.md) | ClipPathElement: clip-path definitions. |
| [ellipse](malight/elements/ellipse.en.md) | EllipseElement, created by pen.ellipse. |
| [group](malight/elements/group.en.md) | GroupElement: bundle elements so they transform and animate together. |
| [image](malight/elements/image.en.md) | ImageElement: a bitmap image. |
| [line](malight/elements/line.en.md) | LineElement, created by pen.line. |
| [link](malight/elements/link.en.md) | LinkElement: click the element to open a URL. |
| [marker](malight/elements/marker.en.md) | MarkerElement: line-end decorations such as arrowheads. |
| [mask](malight/elements/mask.en.md) | MaskElement: control visibility by luminance. |
| [path](malight/elements/path.en.md) | PathElement: move / line / curve / arc commands, turtle drawing and boolean operations. |
| [pattern](malight/elements/pattern.en.md) | PatternElement: tiling fill textures. |
| [polygon](malight/elements/polygon.en.md) | PolygonElement, created by pen.polygon. |
| [polyline](malight/elements/polyline.en.md) | PolylineElement, created by pen.polyline. |
| [rect](malight/elements/rect.en.md) | RectElement, created by pen.rect. |
| [svggroup](malight/elements/svggroup.en.md) | SvgGroupElement: an SVG file imported as an editable group. |
| [svgimage](malight/elements/svgimage.en.md) | SVGImageElement: embed another SVG file as an image. |
...
| Module | Description |
|---|---|
| [editor](malight/pathkit/editor.en.md) | PathEditor: read the structure, drag anchors and control points, insert or drop points, save point data. |
| [htmleditor](malight/pathkit/htmleditor.en.md) | PathHTMLEditor: export a path as one self-contained HTML file you drag points in. |
| [parser](malight/pathkit/parser.en.md) | Parse an SVG path d string into structured segments, and render segments back to d. |
| [point](malight/pathkit/point.en.md) | PathPoint: a single draggable anchor or control point. |
| [segment](malight/pathkit/segment.en.md) | PathSegment: one SVG path command in absolute coordinates, with geometry helpers. |
<!-- DOCS-INDEX:END -->
...
|---|---|---|
| Path helper `pathkit` | Turn a path into **visible points**: inspect the structure, drag anchors/controls, insert or delete points, reverse, scale, save point data | [pathkit/editor.md](malight/pathkit/editor.en.md) ｜ [point.md](malight/pathkit/point.en.md) ｜ [segment.md](malight/pathkit/segment.en.md) |
| Filters `pen.fx` | Photoshop/Illustrator-style filters, chainable, only visually stable effects | [board/fx.md](malight/board/fx.en.md) ｜ [board/filters.md](malight/board/filters.en.md) |
| Extensions `malight.ext` | Add toolkits (charts, icons, effects) to the board without touching the source | [ext.md](malight/ext.en.md) |
| Export engines | Chrome and cairo back ends for PNG and PDF, full path printed | [tools.md](malight/tools.en.md) |
| Enums and constants | Colours, fonts, paper sizes and every option enum | [definitions.md](malight/definitions.en.md) |
| Compatibility | Chinese-API → English-API mapping tables and a one-command migrator | [compat.md](malight/compat.en.md) |
| Internal backend | The tiny SVG element tree and serialiser (rarely needed directly) | [svg_backend.md](malight/svg_backend.en.md) |
...
- **If neither engine is available you get a clear error with two installation
  ...
  The full comparison is in `examples/demo_export.py` and [tools.md](malight/tools.en.md).
...
Mapping tables and all rewrite rules: [compat.md](malight/compat.en.md).
